...
- Polymarket 将已发现事件中的每个可交易市场写入持久粗筛队列；本轮时间用完也不会丢弃队尾，等待期继续分批处理。
  可只暂停该粗筛轮询，恢复后续做；独立的逐市场定时复查不受影响。实时盘口通过公开 WebSocket 订阅；
  低频 HTTP 仍负责市场发现、账户/交易和必要对账，断线或盘口过期时不沿用旧报价。
...
- 另有独立的 `laya` 本地 WebGPU evaluator 插件，默认不启用。先在模型服务页测试连接，再到插件中心的
  “决策评估器”启用并保存服务地址；它与 Jev 可分别选择，不会进入最终交易决策 Provider 池。Laya 服务仅在启动时自动独占测速，也可手动触发；`/health` 返回状态，测速时新推理收到 429，机器人可回退到其他已启用评估器。
