...
<section data-view="settings" hidden><div class="section-intro"><h2>交易风格</h2><p>内置策略按这两个值工作：只做这么多天内揭标的标的，单笔最多买这么多。它要的是小额多次、快进快出——钱压在几个月后才揭标的标的上，就没法去做下一笔，对错也要等几个月才知道。换成你自己的决策策略插件后，这两个值不再起作用；要真正拦住超限的下单，请在插件中心启用业务风控。</p></div><div id="strategySettings" class="plugin-grid"></div><div class="toolbar"><button class="primary" onclick="saveApplicationSettings()">保存交易风格</button><span id="strategySettingsStatus" class="status muted"></span></div></section>
