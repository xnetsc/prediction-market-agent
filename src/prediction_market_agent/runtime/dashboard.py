...
<section><h3>机器人运行控制</h3><p class="muted">管理服务始终可用；全局依赖和相应平台插件配置齐全后，由平台插件自己的事件循环自动运行。暂停状态会持久化。</p><div id="runtimeControl"></div><div class="toolbar"><button class="primary" onclick="saveRuntimeControl()">保存暂停状态</button><button onclick="refreshRuntime()">刷新运行状态</button><span id="runtimeStatus" class="status muted"></span></div></section>
...
<section><h3>程序运行配置</h3><p class="muted">显示全部通用启动配置；这些字段（尤其数据库和工作目录）保存后需重启管理服务。插件选择和插件私有配置保存后会即时重新评估机器人。</p><div id="applicationSettings" class="plugin-grid"></div><div class="toolbar"><button class="primary" onclick="saveApplicationSettings()">保存程序配置</button><button onclick="resetAllApplicationSettings()">删除全部覆盖并恢复默认</button><span id="settingsStatus" class="status muted"></span></div></section>
...
<section><h3>安装新插件</h3><p class="muted">把受信任的 Python 插件源码写入已配置的类别目录。新插件安装后保持禁用，只扫描文件名；启用后才会导入并调用初始化函数。</p><div class="plugin-grid"><label class="field"><b>类别</b><select id="installKind" onchange="renderInstallTargets()"></select></label><label class="field"><b>安装目录</b><select id="installTarget"></select></label><label class="field"><b>插件名</b><input id="installName" placeholder="example_plugin"></label></div><label class="field"><b>Python 源码</b><textarea id="installSource" rows="14" placeholder="def initialize_plugin(context): ..."></textarea></label><div class="toolbar"><button class="primary" onclick="installPlugin()">安装并刷新</button><span id="installStatus" class="status muted"></span></div></section>
...
function renderRuntime(r){let platforms=Object.entries(r.platforms||{}),global='<label><input id="pauseAll" type="checkbox" '+(r.robot_paused?'checked':'')+'> 暂停全部机器人逻辑</label>';let rows=platforms.map(([name,p])=>'<label class="plugin"><input class="pausePlatform" type="checkbox" value="'+esc(name)+'" '+(p.paused?'checked':'')+'> 暂停 '+esc(name)+'<br><span class="'+(p.running?'good':'muted')+'">'+(p.running?'运行中':p.configured?'已停止':'配置不全')+'</span>'+(p.configuration_reasons?.length?'<br><span class="danger">'+esc(p.configuration_reasons.join('；'))+'</span>':'')+'</label>').join('');document.getElementById('runtimeControl').innerHTML=global+'<div class="plugin-grid" style="margin-top:12px">'+rows+'</div><p class="'+(r.global_ready?'good':'danger')+'">'+(r.global_ready?'全局依赖已就绪':esc((r.global_reasons||[]).join('；')))+'</p>'}
async function refreshRuntime(){let r=await get('/api/runtime');renderRuntime(r);return r}
async function saveRuntimeControl(){let s=document.getElementById('runtimeStatus');try{let r=await post('/api/runtime/control',{robot_paused:document.getElementById('pauseAll').checked,paused_platforms:[...document.querySelectorAll('.pausePlatform:checked')].map(e=>e.value)});renderRuntime(r.runtime);s.className='status good';s.textContent='暂停状态已保存并生效'}catch(e){s.className='status danger';s.textContent=e.message}}
...
async function refreshManager(){let [m,r]=await Promise.all([get('/api/plugins/manage'),get('/api/runtime')]);renderManager(m);renderRuntime(r)}
...
async function savePluginConfig(kind,name){let root=document.getElementById('plugin_'+kind+'_'+name),values={};for(let e of root.querySelectorAll('[data-field]')){let v=e.type==='checkbox'?e.checked:e.value;if(e.dataset.type==='integer')v=Number.parseInt(v,10);if(e.dataset.type==='number')v=Number(v);values[e.dataset.field]=v}let s=document.getElementById('manageStatus');try{await post('/api/plugins/config',{kind,name,values});s.className='status good';s.textContent=kind+':'+name+' 配置已保存并重新评估';await refreshManager()}catch(e){s.className='status danger';s.textContent=e.message}}
...
document.getElementById('platform').onchange=refreshAudit;Promise.all([refreshSecurity(),refreshManager(),refreshConfiguration(),refreshAudit()]);setInterval(()=>Promise.all([refreshAudit(),refreshRuntime()]),REFRESH_MS);
</script></body></html>"""


AUTH_HTML = r"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin Passkey</title><style>body{font:16px system-ui;background:#0b1020;color:#e8eefc;display:grid;place-items:center;min-height:100vh}.box{width:min(460px,90vw);background:#151d33;border:1px solid #293453;border-radius:12px;padding:28px}button,input{width:100%;box-sizing:border-box;margin-top:12px;padding:10px;border-radius:7px;border:1px solid #526180;background:#263453;color:#fff}button{background:#325cc7;cursor:pointer}.muted{color:#9fb0d0}.danger{color:#ff8a8a}</style></head><body><div class="box"><h2>ADMIN_TITLE</h2><p class="muted">Passkey 验证同时绑定一次 P-256 ECDH 密钥交换。登录成功后，业务请求和响应均使用该会话密钥加密。</p><input id="name" placeholder="Passkey 名称" ADMIN_NAME><button onclick="begin()">ADMIN_ACTION</button><p id="status" class="danger"></p></div><script>
...
def create_app(config: Config, *, start_robot: bool = True) -> FastAPI:
    ...
    def dispatch(request: Request, session: AdminSession, route: str, body: Any) -> Any:
        ...
        if path == "/api/plugins/config":
            runtime.stop()
            result = management.save_plugin_configuration(str(payload.get("kind", "")), str(payload.get("name", "")), payload.get("values", {}))
            management.refresh()
            if start_robot:
                runtime.reconcile()
            return result
