...
<section data-view="decisions" hidden><div class="section-heading"><div><h3>市场采集与候选分析</h3><p>这里显示已经完成并保存的采集，不代表当前轮次正在取得新数据。删除决策记录不会清除采集证据；模型失败的降级路径也会在这里显示。</p></div><div class="toolbar"><button id="screeningPauseButton" onclick="toggleScreeningPause()" disabled>读取粗筛状态…</button><span id="screeningPauseStatus" class="status muted" role="status"></span></div></div><p class="muted">只暂停评估器粗筛轮询；市场采集、交易决策和已约定时间的单市场复查继续运行。未处理的粗筛队列会保留，恢复后继续。</p><div id="discoveryActivity"><p class="muted">正在读取采集记录…</p></div><details id="evaluatorScreeningsPanel" class="diagnostic-detail"><summary>评估器粗筛明细 <span>逐候选记录，展开后读取</span></summary><p class="muted">只列出已返回并保存的粗筛结果；未粗筛候选不会冒充评估结果，失败调用见上方异常。原始模型请求未单独保存。</p><div id="evaluatorScreenings"></div></details></section>
...
async function saveRuntimeControl(){let s=document.getElementById('runtimeStatus'),root=document.getElementById('runtimeControl');s.className='status pending';s.textContent='正在保存暂停状态…';try{let r=await post('/api/runtime/control',{robot_paused:document.getElementById('pauseAll').checked,paused_platforms:ownPausedPlatforms()});delete root.dataset.dirty;root.dataset.pending='true';renderRuntime(r.runtime);s.className='status good';s.textContent='暂停状态已保存；机器人正在后台切换，进入本页或点击刷新可查看结果'}catch(e){s.className='status danger';s.textContent=e.message}}
