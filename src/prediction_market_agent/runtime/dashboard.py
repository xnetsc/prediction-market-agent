...
<section data-view="settings" hidden class="advanced-section"><div class="section-intro"><h2>统一网络代理</h2><p>OpenRouter、Codex、Claude、平台和研究插件默认继承这里的设置；每个插件仍可选择直连或填写独立代理。</p></div><div id="sharedProxySettings" class="plugin-grid"></div><div class="toolbar"><button class="primary" onclick="saveApplicationSettings()">保存程序配置</button><span id="settingsStatus" class="status muted"></span></div><details><summary>其它程序运行参数 <span>数据库、刷新频率与部署选项</span></summary><p class="muted">大部分选项无需修改。数据库和工作目录须在启动前确定，修改后需重启；公网查询服务与超时在下一次查询生效。</p><div id="applicationSettings" class="plugin-grid"></div><div class="toolbar"><button class="primary" onclick="saveApplicationSettings()">保存程序配置</button><button class="danger" onclick="resetAllApplicationSettings()">恢复全部默认值</button></div></details></section>
...
<section data-view="decisions" id="decisions" hidden><details id="decisionRecordsPanel" class="decision-list-panel"><summary class="section-heading"><div><h3>决策记录</h3><p>先显示最近 10 条，向下滚动每次再加载 5 条。展开列表后，点开一条记录才会读取它的完整证据。</p></div></summary><div id="decisionList"></div></details></section>
<section data-view="decisions" hidden><div class="section-heading"><div><h3>市场采集与候选分析</h3><p>这里显示已经完成并保存的采集，不代表当前轮次正在取得新数据。删除决策记录不会清除采集证据；模型失败的降级路径也会在这里显示。</p></div><div class="toolbar"><button id="screeningPauseButton" onclick="toggleScreeningPause()" disabled>读取粗筛状态…</button><span id="screeningPauseStatus" class="status muted" role="status"></span></div></div><p class="muted">只暂停评估器粗筛轮询；市场采集、交易决策和已约定时间的单市场复查继续运行。未处理的粗筛队列会保留，恢复后继续。</p><details id="discoveryActivityPanel" class="decision-list-panel"><summary class="section-heading"><div><h4 id="discoveryActivitySummary">采集批次与候选记录</h4><p>展开查看各平台采集、分析异常与最近候选。</p></div></summary><div id="discoveryActivity"><p class="muted">正在读取采集记录…</p></div></details><details id="evaluatorScreeningsPanel" class="diagnostic-detail"><summary>评估器粗筛明细 <span>逐候选记录，展开后读取</span></summary><p class="muted">只列出已返回并保存的粗筛结果；未粗筛候选不会冒充评估结果，失败调用见上方异常。原始模型请求未单独保存。</p><div id="evaluatorScreenings"></div></details></section>
...
<script>
const TOKEN='CSRF_TOKEN',SESSION_ID='SESSION_ID',KINDS=['api','decision_provider','decision_evaluator','decision_strategy','market_discovery','research_tool','agent_policy','risk'];
const LOCAL_ACCESS=LOCAL_ACCESS_VALUE;
const CONSOLE_VERSION='__CONSOLE_VERSION__';
const LABELS={api:'交易平台',decision_provider:'AI 模型服务',decision_evaluator:'决策评估器',decision_strategy:'决策策略',market_discovery:'标的发现策略',research_tool:'信息与研究',agent_policy:'Agent 行为风控',risk:'业务风控'};
...
async function saveRuntimeControl(){let s=document.getElementById('runtimeStatus'),root=document.getElementById('runtimeControl');s.className='status pending';s.textContent='正在保存暂停状态…';try{let r=await post('/api/runtime/control',{robot_paused:document.getElementById('pauseAll').checked,paused_platforms:ownPausedPlatforms()});delete root.dataset.dirty;delete root.dataset.pending;renderRuntime(r.runtime);s.className='status '+(r.runtime.settling?'pending':'good');s.textContent=r.runtime.settling?(r.runtime.settling_reason||'设置已保存，正在切换运行状态。'):'暂停设置已保存；当前状态以各平台的运行标识为准。'}catch(e){s.className='status danger';s.textContent=e.message}}
...
async function refreshModelSelectionMode(){const settings=await get('/api/settings');const field=settings.fields.find(f=>f.name==='model_selection_mode');if(field)document.getElementById('modelSelectionMode').value=field.value}
async function saveModelSelectionMode(){const status=document.getElementById('modelSelectionModeStatus'),mode=document.getElementById('modelSelectionMode').value;status.className='status pending';status.textContent='正在保存…';try{await post('/api/settings',{values:{model_selection_mode:mode}});status.className='status good';status.textContent='已保存并用于当前运行；只影响已启用的模型和评估器'}catch(e){status.className='status danger';status.textContent=e.message}}
async function refreshModelServices(){if(!LAST_MANAGER)return refreshManager();let [r]=await Promise.all([get('/api/runtime'),refreshClientControls(),refreshModelSelectionMode()]);renderRuntime(r);renderProviderHealth(r);renderLayaBenchmark(r);return r}
...
async function refreshAudit(){
 ...
 try{
  ...
  let [s,d,m,discovery,screening]=await Promise.all([get('/api/summary'),get('/api/decisions?limit='+LEDGER_FIRST_PAGE+'&offset=0'+dq+ledgerResultsQuery()),get('/api/manifest'),get('/api/discovery/activity?'+q.replace(/^&/,'')),get('/api/screening/control')]);
  renderScreeningControl(screening);
 ...
 }catch(e){document.getElementById('stamp').textContent='错误: '+e;document.getElementById('decisionList').innerHTML='<div class="empty-state"><strong>没能读到决策记录</strong><p>'+esc(String(e&&e.message||e))+'</p></div>';let discovery=document.getElementById('discoveryActivity');if(discovery)discovery.innerHTML='<div class="empty-state"><strong>没能读到采集记录</strong><p>'+esc(String(e&&e.message||e))+'</p></div>'}
...
def _capacity_reason(reading: dict[str, Any]) -> str:
    ...
    now = time.time()
    names = {"claude": "Claude", "codex": "Codex", "openrouter": "OpenRouter"}
...
class AuditData:
    ...
    def discovery_activity(self, platform: str = "") -> dict[str, Any]:
        ...
        connection = sqlite3.connect(self.config.session_db)
        try:
            screening_table = connection.execute(
                "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'market_screening_queue'"
            ).fetchone() is not None
            reviews_table = connection.execute(
                "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'scheduled_market_reviews'"
            ).fetchone() is not None
            ...
            for name, observed_at, batches_count, all_count in batches:
                ...
                if screening_table:
                    row = connection.execute(
                        """SELECT COUNT(*), SUM(last_screened_at > 0),
                                  SUM(next_due_at <= ? AND skip_until <= ?),
                                  SUM(last_error != '')
                ...
                if reviews_table:
                    row = connection.execute(
                        """SELECT MIN(due_at) FROM scheduled_market_reviews
                           WHERE platform = ? AND completed_at = 0""",
                        (name,),
                    ).fetchone()
                    next_review_at = int(row[0]) if row and row[0] is not None else None
                ...
                activity.append(
                    {
                        "platform": str(name),
                        "market_screening_counts": screening_counts,
                        "next_market_review_at": next_review_at,
    ...
    def forget_incidents(self, until_id: int) -> dict[str, Any]:
        """Clear the incidents the operator has read, up to the newest one they were shown.

        These accumulate - every fallback anybody took, for as long as the robot has run - and an
        evidence stream nobody can put down stops being read. Bounded by the id on screen so that
        an incident recorded while the operator was reading is not swept away with the ones they
        had actually seen.
        """
        memory = SessionMemory(self.config.session_db)
        try:
            cursor = memory.connection.execute(
                "DELETE FROM runtime_incidents WHERE id <= ?", (int(until_id),)
            )
            memory.connection.commit()
            return {"deleted": int(cursor.rowcount), "until_id": int(until_id)}
        finally:
            memory.connection.close()
...
def create_app(config: Config, *, start_robot: bool = True) -> FastAPI:
    ...
    def dispatch(request: Request, session: AdminSession | None, route: str, body: Any) -> Any:
        ...
        if path == "/api/discovery/activity":
            ...
            return activity
        if path == "/api/screening/control":
            if body is None:
                return management.screening_control()
            return management.save_screening_control(paused=payload.get("screening_paused"))
        ...
        if path == "/api/runtime/control":
            ...
            runtime.record_saved_pause(result["robot_paused"])
            if start_robot:
        ...
        if path == "/api/settings":
            ...
            result = application_settings.save(payload.get("values", {}))
            runtime.set_model_selection_mode(str(application_settings.values()["model_selection_mode"]))
            return result
        if path == "/api/settings/reset":
            result = application_settings.reset(
        ...
        if path == "/api/laya/probe":
            ...
            try:
                ...
                try:
                    reading = json.loads(error.read(64 * 1024) or b"{}")
        ...
        if path == "/api/plugins/install":
            ...
            if start_robot:
                runtime.reconcile()
            return result
        if path == "/api/plugins/openrouter/create":
            runtime.stop()
            result = management.create_openrouter_plugin(
                str(payload.get("name", "")),
                str(payload.get("target_directory", "")),
            )
            if start_robot:
