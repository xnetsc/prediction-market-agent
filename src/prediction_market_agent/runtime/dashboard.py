...
async function refreshModelServices(){if(!LAST_MANAGER)return refreshManager();let [r]=await Promise.all([get('/api/runtime'),refreshClientControls(),refreshModelSelectionMode()]);renderRuntime(r);renderProviderHealth(r);return r}
...
async function refreshManager(){let [m,r]=await Promise.all([get('/api/plugins/manage'),get('/api/runtime')]);renderManager(m);renderRuntime(r);renderProviderHealth(r);await Promise.all([refreshClientControls(),refreshModelSelectionMode()])}
