...
function renderRuntime(r){
    ...
    if(!root.dataset.dirty&&!root.dataset.pending){
        root.innerHTML='<label class="pause-switch"><input id="pauseAll" type="checkbox" '+(r.robot_paused?'checked':'')+'> 暂停全部平台</label><div class="config-grid">'+Object.entries(r.platforms||{}).map(([name,p])=>'<article class="plugin"><div class="section-heading"><h4>'+esc(name)+'</h4><span class="badge '+(p.running&&!r.robot_paused?'ready':'')+'">'+(p.running&&r.robot_paused?'正在停止旧任务':p.running?(p.runtime&&p.runtime.holding?'已暂停：AI 不可用':'运行中'):p.paused||r.robot_paused?'已暂停':p.ready?'可启动但未运行':'插件报告未就绪')+'</span></div><label><input class="pausePlatform" type="checkbox" value="'+esc(name)+'" '+(p.paused?'checked':'')+'> 暂停此平台</label>'+(!p.ready?'<p><a href="'+configLink('api',name)+'">打开平台插件 →</a></p>':'')+(p.startup_reasons?.length?'<details class="diagnostic-detail"><summary>查看插件报告</summary><p>'+esc(p.startup_reasons.join('；'))+'</p></details>':'')+'</article>').join('')+'</div><p class="status '+(r.global_ready?'good':'muted')+'">'+(r.global_ready?'AI 服务和至少一个平台已报告可启动；是否正常运行以平台状态为准。':'机器人主链尚未满足：需要至少一个可用 AI 服务和一个可启动平台。')+'</p>'+(r.settling?'<p class="status pending">'+esc(r.settling_reason||'正在切换运行状态')+'</p>':'')+(!r.global_ready?'<details><summary>查看具体原因</summary><p>'+esc((r.global_reasons||[]).join('；'))+'</p></details>':'');
        applyPauseAll();
        root.onchange=event=>{
...
function renderSetupGuide(setup){
    const titles={management_only:'界面验收测试实例：未启动机器人进程',not_running:'机器人未运行',pausing:'暂停已保存，正在结束旧任务',paused:'机器人已暂停',partial:'机器人正在运行，但部分平台未运行',running:'机器人正在运行'};
    const descriptions={management_only:'当前访问的是仅用于界面验收的测试实例，未启用机器人运行进程；这不是配置问题。正常启动的实例会在模型服务和至少一个平台就绪且未暂停后立即运行。',not_running:'先处理“启动必需”中的未完成项。AI 服务和至少一个平台报告可启动且未暂停时，平台会立即自动启动。若条件已满足仍未运行，请展开启动错误。',pausing:'不会接收新的业务事件；已经开始的调用需要自行结束。点击“重新检查”查看是否已完全停止。',paused:'当前是用户主动暂停状态，不是缺少必备配置。到本页下方取消暂停并保存后，已就绪平台会立即启动。',partial:'正在运行的平台不受其他平台或可选增强项影响。你可以继续处理未运行的平台，或关闭不使用的平台。',running:'平台扫描已启动，等待市场事件。可选增强项不影响启动；运行中也不代表一定会产生交易或盈利。'};
