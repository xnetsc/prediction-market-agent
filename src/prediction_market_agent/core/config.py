...
APPLICATION_FIELDS = (
    ...
    ApplicationConfigField(
        "strategy_horizon_days", "偏好：多少天内揭标", "integer",
        "内置策略偏好这个天数内结算的标的：钱回得快、错得便宜、效果很快看得见。这是偏好不是禁令——"
        "更远的标的照样会送到 AI 面前，只是排在后面；AI 认为某个远期标的确实更赚钱时可以选，"
        "但必须在理由里说清楚为什么值得压这么久。换成你自己的策略插件时，这个值不再起作用。",
        3, 1, 365,
    ),
    ApplicationConfigField(
        "strategy_max_trade_usdt", "偏好：单笔买多少", "integer",
        "内置策略偏好的单笔金额（计价货币单位，通常是 USDT/USDC）：小额多次、快进快出。"
        "同样是偏好——遇到明显更赚钱的机会，AI 可以下更大的单，但要在理由里讲清楚凭什么。"
        "要不可逾越的硬上限，请在插件中心启用业务风控插件，那是唯一会真正拦下订单的地方。",
        25, 1, 100000000,
    ),
    ApplicationConfigField(
