...
| 两类过滤插件：Agent 行为风控管一切工具调用，业务风控管一切市场 API 动作（含只读） | `core/risk.py`、`runtime/market_guard.py`、`plugins/risk/`、`plugins/agent_policy/` | [RISK_FILTERS.md](RISK_FILTERS.md) | [reject_operation.py](../examples/risk_plugins/reject_operation.py)、[cap_trade_size.py](../examples/risk_rules/cap_trade_size.py) | 净结果、敞口、自定义规则、无插件不加政策及插件边界测试 |
