...
| 功能族 | 实现位置 | 文档 | 例子 | 自动验证 |
|---|---|---|---|---|
| 七类插件自动扫描与插件目录 | `plugin_system/config.py`、`plugin_system/discovery.py` | [PLUGIN_SYSTEM.md](PLUGIN_SYSTEM.md) | [plugin_directories.json](../examples/plugin_directories.json) | [test_plugin_system.py](../tests/test_plugin_system.py)、[test_architecture_contracts.py](../tests/test_architecture_contracts.py) |
| 回环地址免认证、明文业务与公网认证隔离 | `runtime/local_access.py`、`runtime/dashboard.py` | [AUTHENTICATION.md](AUTHENTICATION.md) | `/api/local` JSON 例子见 [CONSOLE_AND_LEDGER.md](CONSOLE_AND_LEDGER.md) | [test_local_access.py](../tests/test_local_access.py)、容器冒烟 |
...
| 从界面安装新插件源码、禁用态发现与刷新 | `plugin_system/management.py`、`runtime/dashboard.py` | [PLUGIN_SYSTEM.md](PLUGIN_SYSTEM.md)、[CONSOLE_AND_LEDGER.md](CONSOLE_AND_LEDGER.md) | [static_demo.py](../examples/api_plugins/static_demo.py) 及其余各类插件例子 | `test_management_service_installs_new_plugin_without_importing_it` |
| 启用/禁用、优先级、刷新、删除后注销、teardown | `plugin_system/managed_config.py`、`plugin_system/management.py` | [PLUGIN_SYSTEM.md](PLUGIN_SYSTEM.md) | 各类示例的 `initialize_plugin` / `teardown` | `test_disabled_file...`、`test_management...`、`test_manual_refresh...` |
...
| Codex、Claude、OpenRouter 与顺序故障转移 | `plugins/providers/`、`agent/decision.py` | [DECISION_PLUGINS.md](DECISION_PLUGINS.md) | [static_provider.py](../examples/decision_provider_plugins/static_provider.py)、[codex.json](../examples/plugin_configs/codex.json) | Provider 优先级、运行期故障转移、OpenRouter schema/代理桥接测试 |
| 每轮输入输出、Agent 步骤、会话滑窗和历史召回 | `runtime/memory.py`、`agent/decision.py` | [DECISION_PLUGINS.md](DECISION_PLUGINS.md)、[CONSOLE_AND_LEDGER.md](CONSOLE_AND_LEDGER.md) | static Provider + static evidence 组合 | `DecisionAndMemoryTests`、`AgentLoopTests` |
...
| 平台插件拥有扫描时机、间隔与失败退避 | `plugins/api/_binance/runtime.py`、`plugins/api/_polymarket/runtime.py` | [ARCHITECTURE.md](ARCHITECTURE.md)、[API_PLUGINS.md](API_PLUGINS.md) | [binance.json](../examples/plugin_configs/binance.json)、[polymarket.json](../examples/plugin_configs/polymarket.json)、[static_demo.py](../examples/api_plugins/static_demo.py) | [test_runtime_lifecycle.py](../tests/test_runtime_lifecycle.py) 的立即运行、中断等待和状态测试 |
| 标准扫描事件进入通用业务循环，框架可主动回调各平台 API | `runtime/events.py`、`runtime/engine.py`、`runtime/controller.py` | [ARCHITECTURE.md](ARCHITECTURE.md) | [static_demo.py](../examples/api_plugins/static_demo.py) 的 `submit_scan` | `RuntimeManagerTests` 的事件提交、处理和暂停测试 |
| 配置就绪自动启动、全局/按平台暂停与状态展示 | `runtime/controller.py`、`plugin_system/managed_config.py`、管理界面 | [CONFIGURATION.md](CONFIGURATION.md)、[CONSOLE_AND_LEDGER.md](CONSOLE_AND_LEDGER.md) | [plugin_selection.json](../examples/plugin_selection.json) | `RuntimeManagerTests`、配置持久化与 HTTP 冒烟 |
...
| 两类过滤插件：Agent 行为风控管一切工具调用，业务风控管一切市场 API 动作（含只读） | `core/risk.py`、`runtime/market_guard.py`、`plugins/risk/`、`plugins/agent_policy/` | [RISK_FILTERS.md](RISK_FILTERS.md) | [reject_operation.py](../examples/risk_plugins/reject_operation.py)、[refuse_large_orders.py](../examples/risk_rules/refuse_large_orders.py)、[refuse_tool.py](../examples/agent_policy_plugins/refuse_tool.py)、[cap_trade_size.py](../examples/agent_policy_rules/cap_trade_size.py) | 净结果、敞口、自定义规则、无插件不加政策及插件边界测试 |
| 同类多插件串链，任一拒绝或抛异常即整体失败 | `core/risk.py` | [RISK_FILTERS.md](RISK_FILTERS.md) | [test_risk_chain.py](../tests/test_risk_chain.py) | 叠加、短路、失败关闭、通配目标、最严格 ADJUST 与链序测试 |
| 专用决策账本与可解释性界面 | `runtime/memory.py`、`runtime/dashboard.py`、`runtime/reporting.py` | [CONSOLE_AND_LEDGER.md](CONSOLE_AND_LEDGER.md) | 已保存 ledger 记录即为可查询样例 | ledger 全链路字段、筛选 API、HTML UI 与后续盘口测试 |
...
| wheel 程序入口与本地/Railway/Vercel/AWS/阿里云部署 | `__main__.py`、`runtime/cloud.py`、根目录与 `deploy/` 部署文件 | [DEPLOYMENT.md](DEPLOYMENT.md) | `start-local.*`、`compose.yaml`、平台模板 | CLI、ASGI、配置解析、wheel 安装、容器与模板验证；契约禁止函数模板替代平台插件调度 |
