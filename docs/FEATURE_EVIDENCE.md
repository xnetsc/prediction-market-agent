...
| 从界面安装新插件源码、禁用态发现与刷新 | `plugin_system/management.py`、`runtime/dashboard.py` | [PLUGIN_SYSTEM.md](PLUGIN_SYSTEM.md)、[HTTP_AUDIT.md](HTTP_AUDIT.md) | [static_demo.py](../examples/api_plugins/static_demo.py) 及其余各类插件例子 | `test_management_service_installs_new_plugin_without_importing_it` |
...
| 配置就绪自动启动、全局/按平台暂停与状态展示 | `runtime/controller.py`、`plugin_system/managed_config.py`、管理界面 | [CONFIGURATION.md](CONFIGURATION.md)、[HTTP_AUDIT.md](HTTP_AUDIT.md) | [plugin_selection.json](../examples/plugin_selection.json) | `RuntimeManagerTests`、配置持久化与 HTTP 冒烟 |
