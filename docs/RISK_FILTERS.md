...
- `portfolio_limits`（业务风控）：插件自己的 JSON 定义资金分配、净结果公式所需阈值、单仓、账户敞口和
  最小订单。它注册两个引擎进业务风控链——按平台的 `market:<平台>` 和全局的 `market:*`——所以它和其它
  业务风控插件一样按顺序串在链上，任意一个拒绝就整体失败。用户原始指定的资本与亏损参数只存在于当前
  私有配置，不是内核或文档默认值。

  它另外提供两件链上判定表达不了的东西，这两件**都不拒绝任何东西**：维护熔断标志，以及在向平台要报价
  **之前**回答"这笔买入最多能下多大"（`allowed_buy_notional`）。后者返回一个数字而不是判定，额度为 0 时
  由动作层记为 `BUY_REJECTED`。

  熔断只挡**增加风险**的动作（买入、转入）。只读、撤单、赎回、卖出、转出在熔断期间照常放行——挡住它们
  会让扫描循环报错退避，并把本该保护的仓位困住。
