...
访问目标是 `localhost`、回环 IP、RFC1918 私网 IPv4（10/8、172.16/12、192.168/16）、
IPv4 链路本地 169.254/16、IPv6 ULA fc00::/7 或链路本地 fe80::/10 时直接进入管理页面，不注册 Passkey、
不创建登录会话、不执行 ECDH/AES-GCM。页面把同样的业务操作以 JSON 发送到 `POST /api/local`。该入口只接受上述本地主机，
拒绝来自其他 Origin 的浏览器请求。判定使用请求 Host，不采用 `X-Forwarded-Host` 或 `X-Forwarded-For`。
本地 Compose 只绑定 `127.0.0.1`；公网反向代理必须验证允许的域名并保留真实公网 Host，不能改成 localhost 或内网 IP。
若自行向局域网开放端口，该网络中能够访问此地址的人无需认证即可管理程序和配置。不要把无鉴权私网入口
转发到公网。普通域名即使解析到私网也不免认证；不以 DNS 查询、代理来源 IP、CGNAT 或保留 IP 推断本地访问。

以下认证流程适用于其他访问主机。客户端 socket IP 可能因容器或反向代理变成内部地址，不用于选择免认证路径。

Web 应用没有默认密码。非上述本地地址访问且认证数据库尚无凭据时，第一个访问者只能进入初始化页，并必须注册首个 admin
...
客户端 OAuth 回调映射验证与管理员认证是两件事。浏览器必须取得当前流程验证入口的随机证明，
且服务端确认本次入口收到过探测，才显示免助手登录。管理页面能访问或某个端口开放都不足以通过。
验证路径和证明每个登录流程独立随机生成，每次探测还带新的 32 字节 challenge，必须由响应原样匹配；
缓存响应和另一个机器人实例无法替代本次证明。仅等待回调期间保留监听，流程结束或超时关闭，
本地部署转发管理进程随后撤销对应临时端口映射；没有固定常驻的回调探测端口。
探测不触发 OAuth 回调、不使用授权码，也不会让公网管理请求免除 Passkey/ECDH。详细流程见
[客户端账号与回调](CLIENT_ACCOUNTS.md)。
