...
Laya 插件内部有有界 FIFO 队列，同一实例至多一个 GPU 请求在执行；排队等待与单次 HTTP/GPU 执行有独立超时，等待超时不会发送请求，执行超时不会产生评估结论。插件实例启动时先做一次合成基准测试（一次预热、三次计时），随后默认每 300 秒测一次；整个测试作为一个独占队列任务，期间该插件的正常请求暂停排队，不会插进测试样本。可通过 `LAYA_BENCHMARK_INTERVAL_SECONDS` 调整，最短 60 秒；单次样本最多等待 15 秒，测试失败只记录错误，不把无效测速当作结果。普通的连接/就绪检查不会启动测速。该独占保证只覆盖机器人通过此插件发起的调用，不能暂停外部程序直接访问 Laya 服务。宿主服务也为已断开客户端仍在运行的 GPU 工作维持独占保护。发现引擎另有一个粗筛消费者，并按实际耗时留短暂、按比例变化的 GPU 间隔；定时复查优先于后台粗筛。对候选粗筛，插件只向模型传必要事实与简短指令，未核实盘口等事实仍按缺失处理，不把截短描述当成拒绝市场的证据。
