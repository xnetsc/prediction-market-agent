...
Laya 插件内部有有界 FIFO 队列，同一实例至多一个请求在执行；排队等待与单次 HTTP/GPU 执行有独立超时，等待超时不会发送请求，执行超时不会产生评估结论。测速现由 Laya 服务自身在启动时及此后每 300 秒执行（一次预热、三次计时），并通过 `/health.benchmark` 公开状态；`POST /benchmark` 可手动触发。服务端等现有推理结束后独占整个测速过程，期间所有新推理立即得到包含测速状态与 `Retry-After` 的 HTTP 429；机器人不再自己发起测速，收到该 429 不盲目重试，而按评估器回退策略处理。普通的连接/就绪检查不会启动测速。宿主服务也为已断开客户端仍在运行的 GPU 工作维持独占保护。发现引擎另有一个粗筛消费者，并按实际耗时留短暂、按比例变化的 GPU 间隔；定时复查优先于后台粗筛。对候选粗筛，插件只向模型传必要事实与简短指令，未核实盘口等事实仍按缺失处理，不把截短描述当成拒绝市场的证据。
