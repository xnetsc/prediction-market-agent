...
服务还可按需加载 Laya Vision。`EVALUATE_FACTS` 的调用形式不变；`state` 可写成
`{"type":"multimodal","text":"…","images":[{"type":"image","media_type":"image/png","data":"<base64>"}]}`。
图片不允许远程 URL，解码后合计不超过 16MB。应用按 `/health.surface.takes.state.kinds` 动态判断是否显示
图片入口；视觉状态只会发给声明 `image`/`multimodal` 的评估器，失败时不会把图片误交给纯文本 Jev。
文本仍走原 Laya 快路径；视觉模型第一次使用才下载约 475MB FP16 导出。同图特征按 SHA-256 复用，文本与
视觉延迟分别出现在 `/health.latency_by_state`。权重许可为 CC BY-NC-SA 4.0，未随服务包或 SDK 捆绑。

Laya 插件内部有有界 FIFO 队列，同一实例至多一个请求在执行；排队等待与单次 HTTP/GPU 执行有独立超时，等待超时不会发送请求，执行超时不会产生评估结论。测速现仅由 Laya 服务自身在启动时自动执行一次，不再定时重复（一次预热、三次计时），并通过 `/health.benchmark` 公开状态；`POST /benchmark` 可手动触发。服务端等现有推理结束后独占整个测速过程，期间所有新推理立即得到包含测速状态与 `Retry-After` 的 HTTP 429；机器人不再自己发起测速，收到该 429 不盲目重试，而按评估器回退策略处理。普通的连接/就绪检查不会启动测速。宿主服务也为已断开客户端仍在运行的 GPU 工作维持独占保护。发现引擎另有一个粗筛消费者，并按刚完成的真实请求耗时留 10%、最少 5ms、最多 500ms 的 GPU 间隔；固定测速只用于状态展示，不参与调度，因为真实候选的序列长度不同。定时复查优先于后台粗筛。对候选粗筛，插件只向模型传必要事实与简短指令，未核实盘口等事实仍按缺失处理，不把截短描述当成拒绝市场的证据。每道问题会与共享状态组成独立的双向编码序列；服务响应和测速状态会报告各序列长度、实际编码 token、编码次数与批处理状态，避免把状态文本长度误当成整次 GPU 工作量。完全相同的序列在同一请求中复用编码结果。
