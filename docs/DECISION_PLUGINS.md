...
插件启动时核对 `/health`：模型须已就绪、运行在 `webgpu`、型号为 `convaiinnovations/laya-multilingual`，并声明
...
Laya 服务是固定的文本评估器，不接受图片或多模态状态。需要视觉输入时，应使用另一个明确声明
`image`/`multimodal` 能力的应用；`EVALUATE_FACTS` 的通用图片协议仍可由这些应用提供。
