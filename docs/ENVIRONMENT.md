...
直连或独立覆盖。OpenRouter 默认继承。通用诊断页面不会覆盖任何配置。未就绪插件显示原因，未启用插件
不为诊断而导入或初始化。
...
初始化上下文提供的解析回调。字段名和读取函数由插件指定，不是通用框架约定。内置 Binance、Polymarket、Codex、Claude、OpenRouter
和标准研究插件均已贡献各自配置路径。该测试探针是独立普通 HTTP 传输，不模拟业务签名、
