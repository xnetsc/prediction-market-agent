...
“刷新账号状态”只读取额度，不发送模型 prompt。Codex 通过官方 App Server 的
`account/rateLimits/read` 读取服务端实际返回的窗口和总可用判定；Claude 读取 Claude Code 使用的
结构化额度服务，临时限流时回退到该客户端最近一次成功缓存。短窗口与周窗口分别显示使用百分比和重置
时间，任一实际存在且生效的窗口耗尽都会标明“额度已用尽”。某些计划（例如只限制周额度的 Codex 计划）
不会返回短窗口，此时页面只显示周额度，不补造短窗口，也不把缺失窗口当成已用尽。Claude Code 仅返回
“正在使用订阅”的说明不包含额度事实，程序不会再把这句话显示成额度状态。

额度读取和客户端请求使用同一账号目录及该 Provider 自己的代理设置，不覆盖程序统一代理或其他插件的
独立代理。界面会标明数据时间；若远端读取暂时失败而使用客户端缓存，也会明确说明是最近一次成功读数。
...
登录等待期间 CLI 子进程必须持续运行，后续验证码提交和用户点击“刷新登录状态”时的查询须回到同一实例；
浏览器不定时轮询。多副本需要会话粘性。
...
macOS/Linux 需要 curl、Bash 和 Python 3.9+，缺少 Python 会明确提示，不自动安装。脚本会在 `python3`
与 `python` 中优先选择已经具有 `cryptography` 的兼容解释器，避免同机多个 Python 时重复安装；若都没有
该依赖，则使用可用解释器创建权限受限的临时 venv，通过 pip 从 PyPI 安装 `cryptography>=49,<51`；
正常结束或信号退出后清理，不改系统 Python。缺少 venv/pip 支持时同样明确报错。Windows 使用系统
PowerShell 5.1+/.NET，不需要 Python。企业脚本策略、代理或防火墙仍可能阻止执行，脚本不会绕过这些限制。
curl/pip 使用本机的相关代理配置；Python 助手的远程 HTTPS 请求使用系统代理设置，PowerShell 使用系统
网络栈，均不会套用服务器容器内的代理地址，也不关闭 TLS 验证。这里的助手运行在浏览器所在宿主机终端：
当“管理页面端点”是已经映射到宿主机的 `localhost/127.0.0.1:<Web 端口>` 时，两种助手明确直连，避免
系统代理自动发现阻塞本机回调。这不适用于“容器访问宿主机 `127.0.0.1` 代理”的场景；后者仍由
[宿主机代理启动脚本](HOST_PROXY.md#容器访问不到时宿主机脚本转发)负责地址改写和必要的转发。
...
客户端默认每六小时检查官方 npm 最新版，间隔可配置；发现新版后后台下载并验证，但必须在界面点击确认才切换。
下载和检查使用对应插件代理。新版暂存在 `clients/<名称>` 的独立版本目录；确认时只原子更新版本指针，
不会覆盖正在运行的客户端或账号数据。同一数据卷即使通过不同容器路径访问，也只会有一个预下载任务；
之前升级中断留下的未验证目录会在下一次持锁预下载时清理，验证失败也会清理本次目录并保留当前版本。
大型平台二进制在慢速网络下可能下载数十分钟；后台下载的 npm 单请求等待与总等待分别最多 30、40 分钟，
不会阻塞配置保存或切断当前客户端，超时会在界面显示失败并允许再次检查。
最多保留待启用新版与
当前可用版，确认后则保留当前新版与上一个已验证可用版；更旧的程序版本目录自动清理。
