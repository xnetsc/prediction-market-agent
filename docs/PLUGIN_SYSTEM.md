...
- `plugins/evaluators/laya.py`：另一个默认禁用的 evaluator 实例，连接外部 WebGPU 服务并验证类型化问答；与 Jev 共用类别，不共用凭据或端点配置。插件内部 FIFO 排队；启动与定时独占测速归随附 Laya 服务负责，状态由 `/health` 返回，测速时推理请求返回带状态的 429。
