...
但**光告诉模型"钱到了"毫无业务价值**：等待期间价格在动，而且**模型是无状态的单轮调用——它根本不记得
自己要过钱**，中间还看了别的市场、下了别的结论。所以这不是"接续会话"，是给一个已经走开的人递一张
**提醒条**，条子上什么都不能省：

| 提醒条上写什么 | 为什么必须有 |
| --- | --- |
| 当时在看哪个市场、哪个 outcome | 它不记得 |
| 当时看到的盘口和剩余时间 | 用来和现在对比 |
| 要了多少、**为什么要**（它自己当时的话） | 这是唯一能判断"还成不成立"的依据 |
| **过了多久** | 等了三小时和等了三十秒，是两件事 |
| 这个市场**还剩多久** | 快结算的和还有两个月的，不是同一笔生意 |
| 资金结果 + 操作员附言 | "我只剩这些了"是给它的指令 |

然后问它一句话：**就这个时间长度和这个资金结果，当初想做的事还值不值得做？**

市场数据是**重新拉的**，不是旧的。硬约束写在策略里：*"Completing a trade you would not open today,
merely because you once started it, is the specific mistake this input exists to prevent."*
